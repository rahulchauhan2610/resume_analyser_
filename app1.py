...
choice = st.sidebar.radio("Welcome,", options=["user", "check_eligibility"])
st.title("RESUME ANALYSER")

# Load the spaCy model (en_core_web_sm)
nlp = spacy.load("en_core_web_sm")  # This is the updated spaCy model
...
# Create the DB
def save_to_database(name, email, contact, skills, resume_file):
    conn = sqlite3.connect('resume_analyzer.db')
    cur = conn.cursor()
    
    # Insert data into the resumes table
    cur.execute('''
        INSERT INTO resumes (name, email, contact, skills, resume_file)
        VALUES (?, ?, ?, ?, ?)
    ''', (name, email, contact, ','.join(skills), resume_file))
